...
export async function getBhawanGalleryImages(bhawanSlug: string): Promise<GalleryImage[]> {
  // TODO: replace with Supabase query
  ...
  const name = nameMap[bhawanSlug] || bhawanSlug
  return MOCK_GALLERY.map((img, idx) => ({
    ...img,
    id: `${bhawanSlug}-gallery-${idx}`,
    caption: `${name} Bhawan — ${img.caption}`,
  }))
}


// --- Notices (per bhawan) ----------------------------------------------------
export async function getBhawanNotices(bhawanSlug: string): Promise<Notice[]> {
  // TODO: replace with Supabase query
  // const { data } = await supabase.from('notices').select('*').eq('bhavan_slug', bhawanSlug).order('posted_at', { ascending: false })
  return [
    {
      id: 'n1',
      bhawanSlug,
    ...
    },
    {
      ...
      body: 'The bhawan gym will now be open from 5:30 AM to 8:00 AM and 6:00 PM to 9:00 PM during summer break.',
      postedAt: '2025-06-01',
      priority: 'normal',
    },
    {
    ...
    },
    {
      id: 'n5',
      bhawanSlug,
      title: 'Inter-Bhawan Sports Registration Open',
      body: 'Registration for the inter-bhawan basketball and table tennis tournament is now open. Fill in the team spreadsheet by June 10th.',
      postedAt: '2025-05-25',
      priority: 'normal',
    ...
    },
    {
      id: 'n6',
      bhawanSlug,
      title: 'Bhawan Cleanliness & Waste Disposal Drive',
